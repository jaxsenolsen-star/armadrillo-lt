...
export function createGame(
  ...
  let hurtFlash = 0;
  let burnCd = 0;
  // "completely stopped" detection: how far the dillo has drifted from an anchor
  let anchorX = p.x;
  let anchorY = p.y;
  let stopTime = 0;
  // fuel warning flash for the HUD
  let dryFlash = 0;
  let wasDry = false;
  let value = 0;
  let maxDepth = 0;
  ...
  function hitTile(tx: number, ty: number, t: number): boolean {
    ...
    if (t >= ORE_BASE) {
      ...
      // Collecting ores costs fuel — no recharging
      p.fuel = Math.max(0, p.fuel - 1.5);
    } else {
  ...
  function update(dt: number) {
    ...
    // Out of fuel = no steering at all. You just roll wherever momentum takes you.
    ...
    // Run ends only when the dillo has COMPLETELY stopped: measured by real
    // travelled distance, not velocity, so a resting bounce still counts as
    // stopped while a slow crawl or a mid-air apex does not.
  ...
  function drawHud() {
    ...
    // Out-of-fuel steering lockout notice
    if (p.fuel <= 0) {
      ...
      ctx.strokeText(msg, 16, 92);
      ctx.fillStyle = "#ff9a9a";
      ctx.fillText(msg, 16, 92);
      ctx.globalAlpha = 1;
    }

    // "About to stop" meter — fills as the dillo sits still
    if (stopTime > 0.22) {
